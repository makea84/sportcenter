from django.test import TestCase
from product.models import Product
from product.forms import ProductForm, StockForm, LineForm
from django.core.files.uploadedfile import InMemoryUploadedFile
from StringIO import StringIO

TEST_IMAGE = '''

'''.strip().decode('base64')


diccionario_producto ={'name':'nombre','description':'descripcion','image':'imagen','price':'100.00'}
diccionario_stock ={'quantity':'10'}
diccionario_linea ={'price':'100.00','quantity':'10','subtotal':'100.00'}
diccionario_factura ={'total':'100.00','date':'01/01/2019'}
    
class ProductTest(TestCase):
    
    def test_valid_form(self):
        image = InMemoryUploadedFile(
            StringIO(TEST_IMAGE),
            field_name='image',
            name='1.JPG',
            content_type='image/jpg',
            size=len(TEST_IMAGE),
            charset='utf-8',
        )
        data ={'name':'nombre','description':'descripcion','price':'100.00'}
        form = ProductForm(data=data, files={'image':image})
        self.assertTrue(form.is_valid())

    def test_invalid_form(self):
        image = InMemoryUploadedFile(
            StringIO(TEST_IMAGE),
            field_name='image',
            name='1.JPG',
            content_type='image/jpg',
            size=len(TEST_IMAGE),
            charset='utf-8',
        )
        data ={'name':'','description':'descripcion','price':'100.00'}
        form = ProductForm(data=data, files={'image':image})
        self.assertFalse(form.is_valid())

        
class StockTest(TestCase):

    def test_valid_form(self):
        image = InMemoryUploadedFile(
            StringIO(TEST_IMAGE),
            field_name='image',
            name='1.JPG',
            content_type='image/jpg',
            size=len(TEST_IMAGE),
            charset='utf-8',
        )
        data ={'name':'nombre','description':'descripcion','price':'100.00','image':image.name}
        producto = Product.objects.create(**data)
        data ={'quantity':'10','product':producto.pk}
        form = StockForm(data=data)
        self.assertTrue(form.is_valid())

    def test_invalid_form(self):
        image = InMemoryUploadedFile(
            StringIO(TEST_IMAGE),
            field_name='image',
            name='1.JPG',
            content_type='image/jpg',
            size=len(TEST_IMAGE),
            charset='utf-8',
        )
        data ={'name':'nombre','description':'descripcion','price':'100.00','image':image.name}
        producto = Product.objects.create(**data)
        data ={'quantity':'','product':producto.pk}
        form = StockForm(data=data)
        self.assertFalse(form.is_valid())

class LineTest(TestCase):

    def test_valid_form(self):
        image = InMemoryUploadedFile(
            StringIO(TEST_IMAGE),
            field_name='image',
            name='1.JPG',
            content_type='image/jpg',
            size=len(TEST_IMAGE),
            charset='utf-8',
        )
        data ={'name':'nombre','description':'descripcion','price':'100.00','image':image.name}
        producto = Product.objects.create(**data)
        data ={'price':'100.00','quantity':'10','subtotal':'1000.00','product':producto.pk}
        form = LineForm(data=data)
        self.assertTrue(form.is_valid())

    def test_invalid_form(self):
        image = InMemoryUploadedFile(
            StringIO(TEST_IMAGE),
            field_name='image',
            name='1.JPG',
            content_type='image/jpg',
            size=len(TEST_IMAGE),
            charset='utf-8',
        )
        data ={'name':'nombre','description':'descripcion','price':'100.00','image':image.name}
        producto = Product.objects.create(**data)
        data ={'price':'','quantity':'10','subtotal':'1000.00','product':producto}
        form = LineForm(data=data)
        self.assertFalse(form.is_valid())

        
