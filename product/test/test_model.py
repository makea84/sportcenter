...
from product.models import Product, Stock, Line, Bill
...
diccionario_producto ={'name':'nombre','description':'descripcion','image':'imagen','price':'100.00'}
diccionario_stock ={'quantity':'10'}
diccionario_linea ={'price':'100.00','quantity':'10','subtotal':'100.00'}
diccionario_factura ={'total':'100.00','date':'01/01/2019'}
...
class ProductTest(TestCase):
    ...
    def create_product(self):
        return Product.objects.create(**diccionario_producto)

    def test_product_creation(self):
        p = self.create_product()
        self.assertTrue(isinstance(p, Product))
        self.assertEqual(p.__unicode__(), diccionario_producto['name'])
        
class StockTest(TestCase):

    def create_stock(self):
        producto = Product.objects.create(**diccionario_producto)
        return Stock.objects.create(product=producto, **diccionario_stock)

    def test_stock_creation(self):
        s = self.create_stock()
        self.assertTrue(isinstance(s, Stock))
        self.assertEqual(s.__unicode__(), s.product.name + '->' + diccionario_stock['quantity'])

class LineTest(TestCase):

    def create_line(self):
        producto = Product.objects.create(**diccionario_producto)
        linea = Line.objects.create(product=producto,**diccionario_linea)
        return linea

    def test_line_creation(self):
        l = self.create_line()
        self.assertTrue(isinstance(l, Line))
        self.assertEqual(l.__unicode__(), l.idLine)
...
class BillTest(TestCase):
    ...
    def create_bill(self):
        producto = Product.objects.create(**diccionario_producto)
        linea = Line.objects.create(product=producto,**diccionario_linea)
        factura = Bill.objects.create(line=linea,**diccionario_factura)
        return factura

    def test_bill_creation(self):
        b = self.create_bill()
        self.assertTrue(isinstance(b, Bill))
        self.assertEqual(b.__unicode__(), b.idBill)
