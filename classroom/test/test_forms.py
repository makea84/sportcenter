from django.test import TestCase
from classroom.models import Classroom, Course, Place, Participation
from classroom.forms import ClassroomForm, CourseForm, PlaceForm, ParticipationForm
from django.contrib.auth.models import User
from django.core.files.uploadedfile import InMemoryUploadedFile
from StringIO import StringIO

TEST_IMAGE = '''

'''.strip().decode('base64')


diccionario_clase ={
    'name':'nombre','width':'100.00','length':'100.00','description':'descripcion',
    'avaible':'Si','image':'imagen','capacity':'01'
}

diccionario_curso ={
    'name':'nombre','day':'Lunes','hour':'09:00','description':'descripcion',
    'duration':'01:00','cost':'100.00'
}
    
class ClassroomTest(TestCase):
    
    def test_valid_form(self):
        image = InMemoryUploadedFile(
            StringIO(TEST_IMAGE),
            field_name='image',
            name='1.JPG',
            content_type='image/jpg',
            size=len(TEST_IMAGE),
            charset='utf-8',
        )
        data ={
            'name':'nombre','width':'100.00','length':'100.00','description':'description',
            'avaible':'Si','capacity':'09'
        }
        form = ClassroomForm(data=data, files={'image':image})
        self.assertTrue(form.is_valid())

    def test_invalid_form(self):
        image = InMemoryUploadedFile(
            StringIO(TEST_IMAGE),
            field_name='image',
            name='1.JPG',
            content_type='image/jpg',
            size=len(TEST_IMAGE),
            charset='utf-8',
        )
        data ={
            'name':'','width':'100.00','length':'100.00','description':'description',
            'avaible':'Si','capacity':'09'
        }
        form = ClassroomForm(data=data, files={'image':image})
        self.assertFalse(form.is_valid())

        
class CourseTest(TestCase):

    def test_valid_form(self):
        data ={
            'name':'nombre','day':'Lunes','hour':'12:00','description':'description',
            'duration':'02:00','cost':'100.00'
        }
        form = CourseForm(data=data)
        self.assertTrue(form.is_valid())

    def test_invalid_form(self):
        data ={
            'name':'','day':'Lunes','hour':'12:00','description':'description',
            'duration':'02:00','cost':'100.00'
        }
        form = CourseForm(data=data)
        self.assertFalse(form.is_valid())

class ParticipationTest(TestCase):

    def create_participation(self):
        usuario = User.objects.create(username='usuario', password='usuario', email='usuario@gmail.com')
        curso = Course.objects.create(**diccionario_curso)
        return Participation.objects.create(user=usuario, course=curso)

    def test_valid_form(self):
        p = self.create_participation()
        data ={
            'user':p.user.pk,'course':p.course.pk,
        }
        form = ParticipationForm(data=data)
        self.assertTrue(form.is_valid())

    def test_invalid_form(self):
        p = self.create_participation()
        data ={
            'user':p.user.pk,'course':'',
        }
        form = ParticipationForm(data=data)
        self.assertFalse(form.is_valid())

        
class PlaceTest(TestCase):

    def create_place(self):
        clase = Classroom.objects.create(**diccionario_clase)
        curso = Course.objects.create(**diccionario_curso)
        return Place.objects.create(classroom=clase, course=curso)

    def test_valid_form(self):
        p = self.create_place()
        data ={
            'classroom':p.classroom.pk,'course':p.course.pk,
        }
        form = PlaceForm(data=data)
        self.assertTrue(form.is_valid())

    def test_invalid_form(self):
        p = self.create_place()
        data ={
            'classroom':p.classroom.pk,'course':'',
        }
        form = PlaceForm(data=data)
        self.assertFalse(form.is_valid())
