$(document).ready(function() {
	...
	$(document).on("click","[class*=boton-adicionar-]",function(){
		var div = $(this).parent();
	...
	$(document).on("click","[class*=boton-quitar-]",function(){
		var div = $(this).parent();
		var form=$(div).find('form');
		console.log(form)
		if ($(form).hasClass('hide')){
	...
	$(document).on("submit","[class*=formulario-adicionar-]",function(event){
		$(this).each(function(){
			event.preventDefault();
			var form = $(this);
			add_element(form);
	...
	$(document).on("submit","[class*=formulario-quitar-]",function(event){
		$(this).each(function(){
			event.preventDefault();
			var form = $(this);
			remove_element(form);
		...
		$.ajax({
			...
			success : function(response_data) {
				if (response_data.hay_error){
            		...
            		var html ='';
            		html+='<div class="row"><div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">';
                    html+='<h2 class="text-success"><strong>Adicion realizada</strong></h2></div></div>';
            		for (var key in response_data) {
            			if(key != 'hay_error'){
            				if (response_data.hasOwnProperty(key)) {
            					html+='<div class="row"><div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">';
		...
		$.ajax({
			...
			success : function(response_data) {
				if (response_data.hay_error){
            		...
            		var html ='';
            		html+='<div class="row"><div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">';
                    html+='<h2 class="text-success"><strong>Borrado realizado</strong></h2></div></div>';
            		for (var key in response_data) {
            			if(key != 'hay_error'){
            				if (response_data.hasOwnProperty(key)) {
            					html+='<div class="row"><div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">';
